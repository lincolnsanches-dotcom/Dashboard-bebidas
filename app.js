// ==========================================
// PROJETO EXCLUSIVO DE BEBIDAS
// ==========================================

// Tenta ler do localStorage primeiro (cache local de bebidas)
let lancamentos = JSON.parse(localStorage.getItem('requisicoes_dados_bebidas')) || [];
let receitasOrcadas = JSON.parse(localStorage.getItem('requisicoes_receitas_orcadas_bebidas')) || {};
let receitasRealizadas = JSON.parse(localStorage.getItem('requisicoes_receitas_realizadas_bebidas')) || {};
let despesasOrcadas = JSON.parse(localStorage.getItem('requisicoes_despesas_orcadas_bebidas')) || {};

// BUSCA AUTOMÁTICA DA NUVEM (FIREBASE - NÓ DE BEBIDAS)
if (typeof firebase !== 'undefined' && firebase.database) {
  firebase.database().ref('dados_dashboard_bebidas').on('value', (snapshot) => {
    ...
    if (dados) {
      ...
      localStorage.setItem('requisicoes_dados_bebidas', JSON.stringify(lancamentos));
      localStorage.setItem('requisicoes_receitas_orcadas_bebidas', JSON.stringify(receitasOrcadas));
      localStorage.setItem('requisicoes_receitas_realizadas_bebidas', JSON.stringify(receitasRealizadas));
      localStorage.setItem('requisicoes_despesas_orcadas_bebidas', JSON.stringify(despesasOrcadas));
...
function sincronizarComFirebase() {
  if (typeof firebase !== 'undefined' && firebase.database) {
    firebase.database().ref('dados_dashboard_bebidas').set({
      ...
      console.log("✅ Dados de BEBIDAS salvos no Firebase!");
    }).catch(err => console.error("❌ Erro ao salvar Bebidas no Firebase:", err));
  }
}
...
const departamentosLista = Array.from(new Set(Object.values(mapeamentoDeptos)));
...
function mudarAba(aba) {
  ...
  [btnLanc, btnBudg, btnDash].forEach(b => {
    if (b) {
      b.className = "px-3 py-1 rounded-lg text-xs font-semibold hover:bg-black/40 text-slate-300 transition cursor-pointer";
    }
  });

  if (aba === 'lancamentos') {
    if (abaLanc) abaLanc.classList.remove('hidden');
    if (btnLanc) btnLanc.className = "px-3 py-1 rounded-lg text-xs font-semibold bg-emerald-500 text-slate-950 shadow transition cursor-pointer";
  } else if (aba === 'budgets') {
    if (abaBudg) abaBudg.classList.remove('hidden');
    if (btnBudg) btnBudg.className = "px-3 py-1 rounded-lg text-xs font-semibold bg-emerald-500 text-slate-950 shadow transition cursor-pointer";
    if (typeof renderizarFormBudgets === 'function') renderizarFormBudgets();
  } else if (aba === 'dashboard') {
    if (abaDash) abaDash.classList.remove('hidden');
    if (btnDash) btnDash.className = "px-3 py-1 rounded-lg text-xs font-semibold bg-emerald-500 text-slate-950 shadow transition cursor-pointer";
    if (typeof atualizarGraficos === 'function') atualizarGraficos();
  }
}
...
function salvarBudgets(e) {
  ...
  alert(`✅ Dados DRE de BEBIDAS salvos com sucesso para ${chaveMes}!`);
}

// IMPORTAÇÃO DA DRE ADAPTADA PARA CONTAS DE BEBIDAS
function importarRelatorioDRE(evento) {
  ...
  reader.onload = function (e) {
    try {
      ...
      const mesAnoInput = document.getElementById('budget-mes-ano');
      const valMesAno = mesAnoInput ? mesAnoInput.value : '';
      const chaveMes = typeof obterChaveMes === 'function' && valMesAno
        ? obterChaveMes(valMesAno) 
      ...
      let lendoReceitaBebidas = false;
      let lendoCustoBebidas = false;
      let leituraMapFapBebidas = false;

      // Trata números vindos diretamente do Excel (preservando decimais)
      function convNumExcel(v) {
        if (v === null || v === undefined || v === '') return 0;
        if (typeof v === 'number') return v;
        let s = String(v).trim();
        // Se for string formatada em PT-BR (ex: "1.733.425,95")
        if (s.includes(',')) {
      ...
      rows.forEach((row) => {
        ...
        const celulaA = row[0] ? String(row[0]).trim() : '';

        // 1. RECEITAS DE BEBIDAS (3.11.102.002)
        if (celulaA.includes('3.11.102.002')) {
          lendoReceitaBebidas = true;
          lendoCustoBebidas = false;
          return;
        }
        if (celulaA.includes('3.11.102.003') || celulaA.includes('Cafe da Manha')) {
          lendoReceitaBebidas = false;
        }

        // MAP/FAP BEBIDAS (3.11.102.005)
        if (celulaA.includes('3.11.102.005')) {
          leituraMapFapBebidas = true;
          return;
        }
        if (celulaA.includes('3.11.107.1') || celulaA.includes('DEDUCOES DA RECEITA BRUTA')) {
          leituraMapFapBebidas = false;
        }

        // 2. CUSTOS DE BEBIDAS (3.12.102.002)
        if (celulaA.includes('3.12.102.002')) {
          lendoCustoBebidas = true;
          lendoReceitaBebidas = false;
          return;
        }

        if (
          celulaA.includes('3.12.102.003') || 
          celulaA.toUpperCase().includes('CAFE DA MANHA') || 
          celulaA.includes('Percentuais de Custo')
        ) {
          lendoCustoBebidas = false;
        }

        // LEITURA DOS DEPARTAMENTOS
        const matchCodigo = celulaA.match(/24\d{2}/);
        if (matchCodigo) {
          const codigo = matchCodigo[0];
          const deptoNome = mapeamentoDeptos[codigo];

          if (deptoNome) {
            // Mapeamento correto das colunas da DRE:
            // row[2] = Coluna C (ORÇADO MÊS ATUAL)
            // row[4] = Coluna E (REAL MÊS ATUAL)
            const valOrcadoMesAtual = convNumExcel(row[2]);
            const valRealizadoMesAtual = convNumExcel(row[4]);

            if (lendoReceitaBebidas || leituraMapFapBebidas) {
              receitasOrcadas[chaveMes][deptoNome] = (receitasOrcadas[chaveMes][deptoNome] || 0) + Math.abs(valOrcadoMesAtual);
              receitasRealizadas[chaveMes][deptoNome] = (receitasRealizadas[chaveMes][deptoNome] || 0) + Math.abs(valRealizadoMesAtual);
            } else if (lendoCustoBebidas) {
              despesasOrcadas[chaveMes][deptoNome] = Math.abs(valOrcadoMesAtual);
            }
          }
        }
      });
      ...
      alert('✅ Relatório DRE importado com sucesso!');
    } catch (err) {
      console.error(err);
      alert('❌ Erro ao processar o arquivo da DRE.');
    }
  };
  ...
  formReq.addEventListener('submit', (e) => {
    ...
    if (tetoDesp > 0) {
      ...
      if (totalMesAtual + valorVal > tetoDesp) {
        ...
        alert(`⚠️ ATENÇÃO: Este lançamento excede o Teto de Despesa de Bebidas do setor (${deptoVal}) em R$ ${formatarMoedaBR(excesso)}!\nO lançamento será gravado para fins de registro.`);
      }
    }
...
function atualizarGraficos() {
  ...
  if (elPerc) {
    if (receitaRealizadaHotel > 0) {
      ...
      elPerc.className = `text-xl font-extrabold mt-0.5 ${cmvReal > 18.0 ? 'text-red-400' : 'text-emerald-400'}`;
    } else {
...
function renderizarInsightsIA(listaAnalise) {
  ...
  // Meta de CMV para Bebidas (exemplo: 18.0%)
  const META_CMV_ALVO = 18.0;

  // PANORAMA GLOBAL BEBIDAS
  if (recOrcTotal > 0 || despOrcTotal > 0) {
    ...
    cardGlobal.innerHTML = `
      <div class="font-bold text-blue-800 flex items-center justify-between mb-1">
        <span class="flex items-center gap-1">🍹 BEBIDAS - PANORAMA FINANCEIRO & SAVING</span>
  ...
  listaAnalise.filter(i => i.despOrc === 0 && i.gasto > 0).forEach(item => {
    ...
    card.innerHTML = `
      <div class="flex justify-between items-start">
        <span class="font-bold text-red-700">🚨 NÍVEL 1: BEBIDAS SEM ORÇAMENTO</span>
        <span class="text-[10px] bg-red-200 text-red-800 px-1.5 py-0.5 rounded font-bold">Sem Teto Aprovado</span>
      </div>
      <p class="mt-1 font-semibold text-gray-800">
        <strong>${item.depto}</strong> registrou <strong>R$ ${formatarMoedaBR(item.gasto)}</strong> em bebidas, mas não possui teto orçado.
      </p>
      ${doador ? `
  ...
  listaAnalise.filter(i => i.despOrc > 0 && i.gasto > i.despOrc).forEach(item => {
    ...
    card.innerHTML = `
      <div class="flex justify-between items-start">
        <span class="font-bold text-red-700">🚨 NÍVEL 1: ORÇAMENTO EXCEDIDO (BEBIDAS)</span>
        <span class="text-[10px] bg-red-200 text-red-800 px-1.5 py-0.5 rounded font-bold">${perc}% do Orçado</span>
      </div>
      <p class="mt-1 font-semibold text-gray-800">
        <strong>${item.depto}</strong> excedeu o orçado de bebidas em <strong>R$ ${formatarMoedaBR(excesso)}</strong>.
      </p>
      ${doador ? `
        <div class="mt-1.5 pt-1.5 border-t border-red-200/60 text-[10px] text-red-900 font-medium">
...
function gerarTextoRelatorioExecutivo() {
  ...
  let texto = `🍹 *RESUMO EXECUTIVO BEBIDAS - CONTROLADORIA DRE*\n`;
  texto += `📅 *Data:* ${dataHojeStr}\n`;
  texto += `-----------------------------------\n\n`;
  
  texto += `📌 *PANORAMA GERAL BEBIDAS*\n`;
  ...
  texto += `\n-----------------------------------\n`;
  texto += `_Relatório gerado automaticamente pelo Sistema de Gestão de Bebidas._`;
...
function exportarRelatorioPDF() {
  ...
  doc.setFontSize(13);
  doc.text("RELATORIO EXECUTIVO BEBIDAS - AUDITORIA E INSIGHTS", 15, 15);
  ...
  const hoje = new Date();
  const nomeArquivo = `Relatorio_Executivo_Bebidas_${hoje.getFullYear()}_${hoje.getMonth() + 1}.pdf`;
